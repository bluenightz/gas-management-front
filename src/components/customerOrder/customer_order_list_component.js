...
class CustomerListOrder extends Component {
  ...
  getDetail() {
    ...
    if(this.state.orderDetail) {
      ...
      return (
        <GasListModalForm onSubmit={this.handleSubmit} type={this.state.type} order={order} />
      );
    } else {
    ...
    return [
      {
        ...
        Cell: ({ original: { _id, orderNumber, status: { name } }}) => <div align="center"><a href="javascript:void(0)" onClick={() => { this.handleClick(_id, name) }} className="text-primary"><u>{orderNumber}</u></a></div>,
      },
      {
  ...
  render() {
    ...
    return <Fragment>
      <BasicList {...this.props} model={this.getModelColumn()} pagename="รายการใบงานแก๊สทั้งหมด" apiPath="/customerOrder" />
      <Modal open={open} onClose={this.closeModal} type={this.state.type}>
        {this.getDetail()}
      </Modal>
    </Fragment>
  }
...
export default withAuth(connect(mapStateToProps, mapDispatchToProps)(CustomerListOrder))
